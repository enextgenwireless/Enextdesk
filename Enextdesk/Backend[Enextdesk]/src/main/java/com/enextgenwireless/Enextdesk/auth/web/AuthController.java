...
import com.enextgenwireless.Enextdesk.auth.domain.LoginRequestDTO;
...
public class AuthController {
    ...
    public String setup(@RequestBody String d) throws JSONException {
        ...
        if (!configurationService.getBoolean(ConfigurationService.JDCONFIG.APP_SETUP)) {
            Login user = new Login("Init", "setup@Enextdeskapp.com", "setup");
            user.setPassword(new BCryptPasswordEncoder().encode("admin"));
            user.setSuperAdmin(true);
            currentLogin.setUser(user);
            Login admin = loginRepo.findByEmailIgnoreCase(data.getString("email"));
            if (null == admin) {
                admin = userService.createUser("Adesoji A", data.getString("email"), data.getString("password"), true, true, false);
            }
